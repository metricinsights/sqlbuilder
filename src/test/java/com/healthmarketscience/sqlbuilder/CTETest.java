...
import static com.healthmarketscience.sqlbuilder.Expressions.cte;
...
public class CTETest extends BaseSqlTestCase {
    ...
    public void testBasicCTE() throws Exception {
        ...
        assertEquals(
                "WITH cte_expr (col1) AS (SELECT t2.col_id FROM Table2 t2) SELECT t0.col1,t1.col2,t2.col5,cte0.col1 FROM Schema1.Table1 t0,Table1 t1,Table2 t2,cte_expr cte0",
                selectStr1);
        ...
        assertEquals(
                "WITH cte_expr (col1) AS (SELECT t2.col_id FROM Table2 t2),cte_expr2 (col2) AS (SELECT t0.col1 FROM Schema1.Table1 t0,cte_expr cte0 WHERE (t0.col2 = cte0.col1)) DELETE FROM cte_expr2 WHERE (col2 > 10)",
                deleteStr1);
    }
    ...
    public void testRecursiveCTE() throws Exception {
        ...
        String selectStr1 = selectQuery1.validate().toString();
        assertEquals(
                "WITH RECURSIVE temp (n,fact) AS (SELECT 0,1 UNION ALL SELECT (cte0.n + 1),((cte0.n + 1) * cte0.fact) FROM temp cte0 WHERE (cte0.n < 9)) SELECT cte0.* FROM temp cte0",
                selectStr1);
    }
